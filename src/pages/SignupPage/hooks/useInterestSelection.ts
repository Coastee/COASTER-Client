...
export const useInterestSelection = () => {
  ...
  const handleCheckboxChange = useCallback((interest: string) => {
    setInterests((prev) => {
      const updatedInterests = prev.includes(interest) ? prev.filter((item) => item !== interest) : [...prev, interest];

      const prevData = JSON.parse(sessionStorage.getItem("signup") || "{}");
      sessionStorage.setItem("signup", JSON.stringify({ ...prevData, interests: updatedInterests }));
  ...
  const handleReset = useCallback(() => {
    setInterests([]);
    const prevData = JSON.parse(sessionStorage.getItem("signup") || "{}");
    sessionStorage.setItem("signup", JSON.stringify({ ...prevData, interests: [] }));
  }, []);
